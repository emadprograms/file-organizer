            ...
            try:
                ...
                try:
                    ...
                    if cg_image:
                        ...
                        if success:
                            ...
                            # Try to match "X من Y" (standard) or "Y X من" (reversed by OCR)
                            match = re.search(r'(\d+)\s*من\s*(\d+)', full_text)
                            if not match:
                                match = re.search(r'(\d+)\s+(\d+)\s*من', full_text)
                            if not match:
                ...
                for attempt in range(3):
                    try:
                        ...
                        # Deterministic Override: If OCR found a footer indicating a subsequent page, force is_continuation = True
                        if extracted_footer:
