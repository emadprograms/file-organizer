    ...
    function renderTimeline() {
        ...
        displayTimeline.forEach(doc => {
            ...
            const highlightClasses = hasNotes 
                ? 'border-l-4 border-l-amber-400 bg-amber-50/40 border-amber-200 shadow-2xs' 
                : 'border-slate-200 hover:border-blue-400';

            card.className = `p-3 bg-white rounded-xl border ${highlightClasses} hover:shadow-sm cursor-pointer transition-all flex flex-col gap-2 group relative`;
            card.draggable = true;
            ...
            const noteBadgeHtml = hasNotes 
                ? `<span class="doc-note-badge text-[10px] bg-amber-100 text-amber-800 border border-amber-300/70 px-1.5 py-0.5 rounded flex items-center gap-1 font-semibold flex-shrink-0" title="Contains custom notes">📝 Note</span>` 
                : '';
            ...
            card.innerHTML = `
                <div class="flex justify-between items-start gap-2">
                    ...
                    <div class="flex items-center gap-1 flex-shrink-0">
                        ${noteBadgeHtml}
                        ${lockBadgeHtml}
                        <button type="button" class="doc-info-btn doc-quick-look-btn opacity-0 group-hover:opacity-100 p-1 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-blue-600 transition-opacity" title="Document Details & Notes (Spacebar)">
            ...
            const infoBtn = card.querySelector('.doc-info-btn') || card.querySelector('.doc-quick-look-btn');
