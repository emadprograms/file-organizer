# Requirements Archive: v2.0 Logic-Based Modular Refactoring
...
- [x] **TEST-01**: All test files in `tests/` must be proper `pytest` or `unittest` modules with descriptive names following a strict `test_{module}_{what_is_tested}.py` convention (e.g. `test_utils_telemetry_audit.py`). All old test files must be renamed or converted appropriately.
...
- [x] **ARCH-02**: Migrate all existing files into their new appropriate locations (except standalone utility scripts like `rotate_process.py` which can remain in root).
...
- [x] **YAML-01**: Create `tenant_config/yaml_loader.py` to check the root folder for a "source files" directory and read the YAML configuration.
