...
- [x] **QCK-38**: Vacant House Grey Styling & False Active Tenant Fallback Removal:
  - When all tenants have vacated, backend returns `current_tenant: null`, `duration_category: null`, and `tenure_color: "grey"`.
  - Area Grid cards render grey left border (`border-l-slate-300`), a `Vacant` pill badge, and neutral slate styling on past tenants without colored duration badges.
- [x] **QCK-39**: Instant Cross-Tenant Document Move & Folder Lifecycle:
  - Immediate DOM removal (`removeDocFromDom`) when moving single or batch documents to another tenant, auto-closing empty folders and decrementing count badges without manual reload.
- [x] **QCK-40**: Vacated Tenant Document Date Conflict & Tenancy Extension Prompt:
  - Detects when ingesting a document dated after a tenant's vacate date. Backend returns HTTP 400 Bad Request prompting user with exact date discrepancy.
  - Interactive `#vacated-tenant-modal` enables extending tenancy end date, uploading without extending, or cancelling.
- [x] **QCK-41**: Past Tenants Chronological Sorting by Vacate Date (`end_date DESC`, `start_date DESC`):
  - Orders past tenants by their vacate date so the most recent past resident (e.g. vacated in 2024) is listed first rather than older tenants with later start dates.
  - Vacant house subtitles accurately reflect the latest occupancy span (e.g. `2000 - 2024`).
- [x] **QCK-42**: Header Bar Tenure Legend `Vacant` Indicator & Full Dark Mode Support:
  - Pinned `● Vacant` slate grey marker (`bg-slate-400 dark:bg-slate-500`) into `#grid-tenure-legend` alongside `< 5y`, `5–10y`, and `> 10y` with complete dark mode styling.
- [x] **VER-07**: Comprehensive multi-stack test suite covering all capabilities across Python and .NET:
  - 148 ASP.NET Core xUnit tests (`web-net/FileOrganizer.Tests/`, 100% passing).
  - 44 Python pytest tests (`tests/test_document_management_api.py`, `tests/test_v14_features.py`, `tests/test_api_v11.py`).
  - 277 Frontend Vitest tests across 27 files (`npm run test:frontend`, 100% passing).
...
| **QCK-38** | Vacant House Grey Styling & False Active Tenant Fallback Removal | QCK-38 | Complete | `RepositoryTests.cs`, `test_api_v11.py`, `area_grid_card.test.js`, zero static diff |
| **QCK-39** | Instant Cross-Tenant Document Move & Folder Lifecycle | QCK-39 | Complete | `move_to_other_tenant.test.js` (7 tests), `categories-view.js`, zero static diff |
| **QCK-40** | Vacated Tenant Document Date Conflict & Tenancy Extension Prompt | QCK-40 | Complete | `ApiEndpointTests.cs`, `test_api_v11.py`, `ingest_station.test.js` (3 tests), zero static diff |
| **QCK-41** | Past Tenants Chronological Sorting by Vacate Date (`end_date DESC`) | QCK-41 | Complete | `RepositoryTests.cs`, `test_api_v11.py`, `FileOrganizerRepository.cs`, `routes.py` |
| **QCK-42** | Header Bar Tenure Legend `Vacant` Indicator & Full Dark Mode Support | QCK-42 | Complete | `unified_header.test.js`, `index.html` across all 3 webroots |
| **VER-07** | Comprehensive multi-stack automated testing suite (Pytest, Vitest, xUnit) | Phase 108 | Complete | 469 automated tests passing across 3 test runners (148 xUnit, 44 pytest, 277 Vitest across 27 files); zero static asset diff. |
