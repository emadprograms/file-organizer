...
- Allow the local LLM to guess the document type utilizing these specific patterns:
  ...
  - Contains contract articles like "مادة (1)", "مادة (2)", "الطرف الأول" (First Party), "الطرف الثاني", or "التمهيد" (Preamble) -> contract. (WARNING: Contract clauses often mention rent deduction or eviction. They MUST remain contract and NEVER be classified as rent deduction or notifications).
  ...
  - Contains text for "rent deduction" (e.g., استقطاع الإيجار) or mentions deducting amounts like "30 bd" or "60 bd" -> rent deduction. Usually formatted as letters. WARNING: Contracts and basic_details profile forms are EXEMPT and can contain rent amounts without being rent_deduction.
