...
def extract_all_page_images(pdf_path):
    ...
    for i in range(len(doc)):
        ...
        images.append((i+1, image_base64))
    doc.close()
    return images

def test_qwen_model(client, model, image_base64):
...
def main():
    ...
    print(f"Extracting images from all pages of {pdf_path}...")
    images = extract_all_page_images(pdf_path)
    print(f"Extracted {len(images)} pages.")

    groq_key = os.getenv("GROQ_API_KEY")
    if not groq_key:
        print("❌ API keys missing in .env (Check GROQ_API_KEY)")
        return
    ...
    print(f"\nStress Testing Qwen model: {model} across {len(images)} pages")
    print("=" * 50)

    with open("groq_multi_model_test.log", "w") as f:
        f.write(f"Stress Testing Qwen model: {model} across {len(images)} pages\n")
        f.write("=" * 50 + "\n")

    for page_num, image_base64 in images:
        print(f"Testing Page {page_num}...")
        result, duration = test_qwen_model(groq_client, model, image_base64)
        
        output_str = f"Page {page_num} - Response Time: {duration:.2f}s\nOutput:\n{result}\n" + "-" * 50 + "\n"
        print(output_str)
        
        with open("groq_multi_model_test.log", "a") as f:
            f.write(output_str)
