...
(function() {
    ...
    function renderHouseProfile(profile) {
        ...
        const tenantCount = profile.tenants.length;
        const tenantLabel = tenantCount === 1 ? 'مستأجر' : 'مستأجرين';
        tenantsHeader.innerHTML = `
            ...
            <span class="tenants-count-badge px-2.5 py-0.5 inline-flex items-center justify-center text-[11px] font-semibold text-slate-600 bg-slate-100 border border-slate-200 rounded-full">
                ${tenantCount} ${tenantLabel}
            </span>
        `;
